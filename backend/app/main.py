...
async def lifespan(app: FastAPI):
    logger.info("CONCURIS backend starting up", env=settings.env)
    # Create all tables (use Alembic in production)
    # async with engine.begin() as conn:
    #     await conn.run_sync(Base.metadata.create_all)
    yield
    logger.info("CONCURIS backend shutting down")


def create_app() -> FastAPI:
    app = FastAPI(
        title="CONCURIS API",
        description="Backend API for CONCURIS — Construction Company/Worker/Vendor/Public-AI Platform",
        version="0.1.0",
        lifespan=lifespan,
    )
